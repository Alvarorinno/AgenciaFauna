...
export default function Eventos() {
  ...
  useEffect(() => {
    getCotizaciones().then(data => { setRows(data); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  ...
  async function handleAdd() {
    const maxNCot = rows.reduce((m, r) => Math.max(m, r.n_cot || 0), 0);
    const created = await createCotizacion({ n_cot: maxNCot + 1, mes: 'enero' });
